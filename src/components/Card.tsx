      ...
      <CardFooter>
        <div className="d-flex justify-content-between align-items-baseline">
          ...
          <label className="d-flex align-items-baseline">
            <input className="m-2" type="checkbox" onClick={handleDoStateChange} />
            <div className="fst-italic">{doTask}</div>
          </label>
        </div>
        <CardHandle isModal={editModal} handleToggle={() => setEditModal(false)} taskObj={taskObj} isCreated={false} isEdited={true} />
        <DeleteTask modal={deleteModal} toggle={() => setDeleteModal(false)} task={taskObj} />
      </CardFooter>
    </Card>
  );
};
